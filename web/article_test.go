...
import (
	...
	"github.com/toormi/go-clean-arch/web/mocks"
	"net/http"
	"net/http/httptest"
	"strconv"
	"strings"
...
func TestFetch(t *testing.T) {
	var mockArticle persistence.Article
	err := faker.FakeData(&mockArticle)
	assert.NoError(t, err)
	mockUCase := new(mocks.ArticleService)
	mockListArticle := make([]persistence.Article, 0)
...
func TestFetchError(t *testing.T) {
	...
	num := 1
	cursor := "2"
	mockUCase.On("Fetch", mock.Anything, cursor, int64(num)).Return(nil, "", web.ErrInternalServerError)
