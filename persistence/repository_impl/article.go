...
func (m *ArticleRepository) fetch(ctx context.Context, query string, args ...interface{}) (result []persistence.Article, err error) {
...
func (m *ArticleRepository) Fetch(ctx context.Context, cursor string, num int64) (res []persistence.Article, nextCursor string, err error) {
	query := `SELECT id,title,content, author_id, updated_at, created_at
  						FROM article WHERE created_at > ? ORDER BY created_at LIMIT ? `

	decodedCursor, err := DecodeCursor(cursor)
	if err != nil && cursor != "" {
		return nil, "", web.ErrBadParamInput
	}
...
func (m *ArticleRepository) GetByID(ctx context.Context, id int64) (res persistence.Article, err error) {
	query := `SELECT id,title,content, author_id, updated_at, created_at
	...
	if len(list) > 0 {
		...
		return res, web.ErrNotFound
	}

	return
}

func (m *ArticleRepository) GetByTitle(ctx context.Context, title string) (res persistence.Article, err error) {
	query := `SELECT id,title,content, author_id, updated_at, created_at
...
func (m *ArticleRepository) Store(ctx context.Context, a *persistence.Article) (err error) {
...
func (m *ArticleRepository) Update(ctx context.Context, ar *persistence.Article) (err error) {
